...
def generate_thumbnail(video_data, original_filename):
    """Generate thumbnail from video data and upload to Supabase"""
    if not supabase:
        print("Supabase not available for thumbnail generation")
        return None
...
def upload_file():
    try:
        ...
        if not file_url:
            return jsonify({'error': 'Failed to upload to cloud storage. Check Supabase configuration.'}), 500
...
def delete_file(filename):
    try:
        ...
        if delete_from_supabase(secure_name):
