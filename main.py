...
def index():
    try:
        return send_from_directory(app.static_folder, 'index_with_all_content.html')
    except Exception as e:
        ...
        <body>
            ...
            <br><a href="/documents">View Documents</a>
...
def admin():
    ...
    try:
        ...
        <body>
            <h1>Admin Panel</h1>
            <p>Status: {status}</p>
            <p>Error loading admin panel: {str(e)}</p>
        </body>
        </html>
        '''
...
def public_documents():
    """Public page to view documents - downloads disabled"""
    try:
        ...
        for doc in documents:
            ...
            <div style="border: 1px solid #ddd; padding: 15px; margin: 10px 0; border-radius: 5px; background: #f9f9f9;">
                <h3><i class="{icon}" style="margin-right: 10px; color: #999;"></i>{doc.title}</h3>
                <p>{doc.description or 'No description available'}</p>
                <p><small>Size: {size_mb} MB | Views: {doc.download_count} | Added: {doc.created_at.strftime('%Y-%m-%d')}</small></p>
                <button disabled style="background: #e5e7eb; color: #9ca3af; padding: 8px 15px; border: none; border-radius: 3px; cursor: not-allowed;">
                    <i class="fas fa-ban"></i> Download Disabled
                </button>
                <small style="color: #999; margin-left: 10px;">Contact administrator for access</small>
            </div>
            '''
        ...
        <body>
            ...
            <a href="/" style="color: #007cba;">← Back to Home</a>
            <div class="notice">
                <strong>📢 Notice:</strong> Document downloads are currently disabled. Please contact an administrator for access to download documents.
            </div>
            <hr>
            {doc_list}
        </body>
...
def download_document(doc_id):
    """Disabled download route - returns error message"""
    return '''
    <!DOCTYPE html>
    <html>
    <head><title>Download Disabled</title></head>
    <body style="text-align:center; padding:50px; font-family:Arial;">
        <h2>🚫 Downloads Disabled</h2>
        <p>Document downloads are currently disabled for security reasons.</p>
        <p>Please contact an administrator if you need access to this document.</p>
        <a href="/documents">← Back to Documents</a>
    </body>
    </html>
    ''', 403

# NEW: Article download route for admin
@app.route('/api/download/article/<int:article_id>')
def download_article(article_id):
    """Download article as text file (admin only)"""
    try:
        # Simple admin check - in production, use proper authentication
        admin_key = request.args.get('admin_key')
        correct_password = os.environ.get('ADMIN_PASSWORD', 'admin123')
        
        if not admin_key or admin_key != correct_password:
...
"""
        ...
        print(f"📥 Article downloaded: {article.title} (ID: {article.id})")
        return response
...
def health():
    return jsonify({
        ...
        'article_downloads': 'enabled_for_admin',
        'supabase': {
    ...
    try:
        if request.method == 'GET':
            ...
            return jsonify([{
                ...
                'downloads_enabled': False  # Always disabled on main site
            } for d in documents])
...
def manage_document(doc_id):
    try:
        ...
        if request.method == 'GET':
            ...
            # Optionally delete file from Supabase here
            filename = document.filename
...
with app.app_context():
    try:
        ...
        print("✅ Article downloads enabled for admin")
        print("❌ Document downloads disabled for public")
        print("✅ Video management with thumbnails")
        print("✅ File upload to Supabase")
        print("✅ Admin panel with password protection")
