...
# Initialize Supabase with detailed error reporting
supabase = None
supabase_error = None
...
try:
    ...
    print(f"✓ Supabase connection test successful: {len(result)} buckets found")
    ...
    if SUPABASE_BUCKET in bucket_names:
        ...
        print(f"❌ Bucket '{SUPABASE_BUCKET}' not found. Available buckets: {bucket_names}")
        
except ImportError as e:
    supabase_error = f"Import error: {e}"
    print(f"❌ {supabase_error}")
except ValueError as e:
    supabase_error = f"Configuration error: {e}"
    print(f"❌ {supabase_error}")
except Exception as e:
    supabase_error = f"Connection error: {e}"
    print(f"❌ {supabase_error}")

print("=== END SUPABASE DEBUG ===")
...
def upload_to_supabase(file_data, filename, content_type):
    ...
    if not supabase:
        error_msg = f"Supabase not available: {supabase_error}"
        print(f"❌ {error_msg}")
        return None
    
    try:
        print(f"📤 Uploading {filename} to bucket '{SUPABASE_BUCKET}'...")
...
def generate_thumbnail(video_data, original_filename):
    ...
    if not supabase:
        print(f"❌ Supabase not available for thumbnail: {supabase_error}")
        return None
...
def list_supabase_files():
    ...
    if not supabase:
        print(f"❌ Supabase not available for listing: {supabase_error}")
        return []
...
@app.route('/')
def index():
    status = "Supabase Connected ✓" if supabase else f"Supabase Error: {supabase_error}"
    try:
...
def admin():
    try:
        ...
        <body>
            <h1>Admin Panel</h1>
            <p>Status: {status}</p>
            <small>Static file error: {str(e)}</small>
        </body>
        </html>
        '''
...
@app.route('/health')
def health():
    # Test bucket access if Supabase is available
    bucket_test = None
    if supabase:
        try:
            result = supabase.storage.list_buckets()
            ...
            bucket_test = {
                ...
                'target_bucket_exists': SUPABASE_BUCKET in bucket_names
            }
        except Exception as e:
            bucket_test = {'error': str(e)}
    ...
    return jsonify({
        ...
        'supabase': {
            ...
            'url_value': SUPABASE_URL if SUPABASE_URL else 'NOT SET',
            'key_preview': SUPABASE_KEY[:20] + '...' if SUPABASE_KEY else 'NOT SET',
    ...
    try:
        ...
        if not file_url:
            return jsonify({
                ...
                'bucket': SUPABASE_BUCKET,
                'debug': 'Check /health endpoint for detailed diagnostics'
            }), 500
