...
# NEW ROUTE: Article viewing (this is what was missing!)
...
<head>
    ...
    <style>
        ...
        .meta {{ color: #666; font-size: 14px; margin: 20px 0; padding: 10px 0; border-bottom: 1px solid #eee; }}
        .content {{ margin: 30px 0; font-size: 16px; color: #444; }}
        .download {{ background: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block; margin-top: 20px; }}
...
<body>
    ...
    <h1>{article.title}</h1>
    <div class="meta">Published on {article.created_at.strftime('%B %d, %Y')}</div>
    <div class="content">{content}</div>
    <a href="/api/download/article/{article.id}" class="download">Download as Text File</a>
</body>
</html>"""
    except Exception as e:
        return f"<h1>Error</h1><p>Could not load article: {str(e)}</p><a href='/'>← Back to Home</a>", 500
    ...
    try:
        ...
        for doc in documents:
            ...
            <div style="border: 1px solid #ddd; padding: 15px; margin: 10px 0; border-radius: 5px;">
                <h3><i class="{icon}" style="margin-right: 10px;"></i>{doc.title}</h3>
                ...
                <a href="/download/{doc.id}" style="background: #007cba; color: white; padding: 8px 15px; text-decoration: none; border-radius: 3px;">
...
# Keep existing routes for videos and texts
